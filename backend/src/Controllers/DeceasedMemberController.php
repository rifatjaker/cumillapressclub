...
final class DeceasedMemberController
{
    public function publicIndex(Request $request): array
    {
        ...
        $stmt = $pdo->query(
            'SELECT id, name, role, tenure, photo_path, sort_order
    ...
    public function adminIndex(Request $request): array
    {
        ...
        $stmt = $pdo->query(
            'SELECT id, name, role, tenure, photo_path, sort_order, is_active, created_at, updated_at
             FROM deceased_members
             ORDER BY sort_order ASC, id ASC'
        );
    ...
    public function store(Request $request): array
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO deceased_members
             (name, role, tenure, photo_path, sort_order, is_active, created_by, updated_by)
             VALUES
             (:name, :role, :tenure, :photo_path, :sort_order, :is_active, :created_by, :updated_by)'
        );
        $stmt->execute([
        ...
        return [
            'data' => [
                'success' => true,
                'message' => 'Deceased member created',
    ...
    public function update(Request $request, array $params): array
    {
        ...
        if ($id <= 0) {
            return [
                'data' => ['success' => false, 'message' => 'Invalid deceased member id'],
                'status' => 422,
            ];
        }
        ...
        $findStmt = $pdo->prepare('SELECT photo_path FROM deceased_members WHERE id = :id LIMIT 1');
        $findStmt->execute(['id' => $id]);
        $existing = $findStmt->fetch(PDO::FETCH_ASSOC);

        if (!$existing) {
            return [
                'data' => ['success' => false, 'message' => 'Deceased member not found'],
                'status' => 404,
            ];
        }
        ...
        return [
            'data' => [
                'success' => true,
                'message' => 'Deceased member updated',
            ],
            'status' => 200,
        ];
    }
    ...
    public function destroy(Request $request, array $params): array
    {
        ...
        $deleteStmt = $pdo->prepare('DELETE FROM deceased_members WHERE id = :id');
        ...
        return [
            'data' => [
                'success' => true,
                'message' => 'Deceased member deleted',
            ],
            'status' => 200,
        ];
    }
    ...
    /** @return array{name: string, role: string, tenure: string, sort_order: int, is_active: int} */
    ...
    private function saveUploadedPhoto(array $file): array
    {
        ...
        $uploadDir = BASE_PATH . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'deceased';
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
            return ['success' => false, 'message' => 'Cannot create upload directory'];
        }

        $fileName = 'deceased-' . bin2hex(random_bytes(12)) . '.' . $extension;
        ...
        return [
            'success' => true,
            'path' => '/uploads/deceased/' . $fileName,
        ];
    }
