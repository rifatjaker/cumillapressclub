...
final class CommitteeController
{
    public function publicIndex(Request $request): array
    {
        ...
        $stmt = $pdo->query(
            'SELECT id, name, role, message, phone, email, social, media, photo_path, sort_order
    ...
    public function adminIndex(Request $request): array
    {
        ...
        $stmt = $pdo->query(
            'SELECT id, name, role, message, phone, email, social, media, photo_path,
    ...
    public function store(Request $request): array
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO committee_members
             (name, role, message, phone, email, social, media, photo_path, sort_order, is_active, created_by, updated_by)
             VALUES
             (:name, :role, :message, :phone, :email, :social, :media, :photo_path, :sort_order, :is_active, :created_by, :updated_by)'
        );
        $stmt->execute([
    ...
    public function update(Request $request, array $params): array
    {
        ...
        if ($id <= 0) {
            return [
                'data' => ['success' => false, 'message' => 'Invalid committee id'],
                'status' => 422,
            ];
        }
        ...
        $findStmt = $pdo->prepare('SELECT photo_path FROM committee_members WHERE id = :id LIMIT 1');
        $findStmt->execute(['id' => $id]);
        $existing = $findStmt->fetch(PDO::FETCH_ASSOC);

        if (!$existing) {
            return [
                'data' => ['success' => false, 'message' => 'Committee member not found'],
                'status' => 404,
            ];
        }
    ...
    public function destroy(Request $request, array $params): array
    {
        ...
        $deleteStmt = $pdo->prepare('DELETE FROM committee_members WHERE id = :id');
        ...
        return [
            'data' => [
                'success' => true,
                'message' => 'Committee member deleted',
            ],
            'status' => 200,
        ];
    }
    ...
    /** @return array{name: string, role: string, message: string, phone: string, email: string, social: string, media: string, sort_order: int, is_active: int} */
    ...
    private function saveUploadedPhoto(array $file): array
    {
        ...
        $uploadDir = BASE_PATH . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'committee';
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
            return ['success' => false, 'message' => 'Cannot create upload directory'];
        }

        $fileName = 'committee-' . bin2hex(random_bytes(12)) . '.' . $extension;
        ...
        return [
            'success' => true,
            'path' => '/uploads/committee/' . $fileName,
        ];
    }
