...
export default function LeadershipManager({ token }) {
  ...
  const loadItems = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/admin/leadership-profiles`, {
        headers: {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || 'নেতৃত্বের প্রোফাইল লোড করা যায়নি')
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      formData.append('photo_tag', form.photo_tag)
      ...
      const url = editingId
        ? `${API_BASE}/api/v1/admin/leadership-profiles/${editingId}`
        : `${API_BASE}/api/v1/admin/leadership-profiles`
      ...
      setMessage(editingId ? 'প্রোফাইল আপডেট হয়েছে' : 'নতুন প্রোফাইল যোগ হয়েছে')
      resetForm()
      await loadItems()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('এই নেতৃত্বের প্রোফাইল মুছে ফেলবেন?')) {
      return
    }
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/admin/leadership-profiles/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
      ...
      setMessage('প্রোফাইল মুছে ফেলা হয়েছে')
      await loadItems()
    } catch (err) {
  ...
  return (
    <section className="mt-6 rounded-3xl border border-ink/10 bg-white p-5 shadow-card dark:border-white/20 dark:bg-[#101827]">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-lg font-bold text-ink dark:text-white">নেতৃত্বের প্রোফাইল</h3>
          <p className="text-sm text-ink/70 dark:text-white/70">সভাপতি, সম্পাদকসহ নেতৃত্বের তথ্য ও ছবি ম্যানেজ করুন</p>
        </div>
        <button
          type="button"
          onClick={loadItems}
      ...
      <div className="grid gap-5 lg:grid-cols-5">
        <form className="space-y-2 rounded-2xl border border-ink/10 bg-ink/[0.02] p-4 dark:border-white/15 dark:bg-white/5 lg:col-span-2" onSubmit={handleSubmit}>
          <h4 className="text-sm font-bold text-ink dark:text-white">{editingId ? 'প্রোফাইল এডিট' : 'নতুন প্রোফাইল'}</h4>
          <input
            required
            value={form.name}
          ...
          <input
            ...
            placeholder="Facebook / Social"
            className="w-full rounded-lg border border-ink/20 bg-white px-3 py-2 text-sm outline-none dark:border-white/25 dark:bg-white/10 dark:text-white"
          />
          <input
            value={form.media}
            onChange={handleChange('media')}
            placeholder="মিডিয়া হাউজ (ঐচ্ছিক)"
            className="w-full rounded-lg border border-ink/20 bg-white px-3 py-2 text-sm outline-none dark:border-white/25 dark:bg-white/10 dark:text-white"
          />
          <input
            value={form.photo_tag}
            onChange={handleChange('photo_tag')}
            placeholder="ফটো ট্যাগ (ঐচ্ছিক)"
            className="w-full rounded-lg border border-ink/20 bg-white px-3 py-2 text-sm outline-none dark:border-white/25 dark:bg-white/10 dark:text-white"
          />
          <input
            type="number"
          ...
          <table className="min-w-full border-collapse text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-2 py-4 text-center text-ink/65 dark:text-white/70">কোনো প্রোফাইল নেই</td>
                </tr>
              ) : (
                items.map((item) => (
