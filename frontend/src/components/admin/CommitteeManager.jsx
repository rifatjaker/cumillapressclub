...
export default function CommitteeManager({ token }) {
  ...
  const loadItems = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/admin/committee-members`, {
        headers: {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || 'কমিটি সদস্য লোড করা যায়নি')
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const url = editingId
        ? `${API_BASE}/api/v1/admin/committee-members/${editingId}`
        : `${API_BASE}/api/v1/admin/committee-members`
      ...
      setMessage(editingId ? 'কমিটি সদস্য আপডেট হয়েছে' : 'নতুন কমিটি সদস্য যোগ হয়েছে')
      resetForm()
      await loadItems()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('এই কমিটি সদস্য মুছে ফেলবেন?')) {
      return
    }
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/admin/committee-members/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
      ...
      setMessage('কমিটি সদস্য মুছে ফেলা হয়েছে')
      await loadItems()
    } catch (err) {
  ...
  return (
    <section className="mt-6 rounded-3xl border border-ink/10 bg-white p-5 shadow-card dark:border-white/20 dark:bg-[#101827]">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-lg font-bold text-ink dark:text-white">বর্তমান নির্বাহী কমিটি</h3>
          <p className="text-sm text-ink/70 dark:text-white/70">কমিটির সদস্যদের নাম, পদবী, মিডিয়া, ফোন ও ছবি ম্যানেজ করুন</p>
        </div>
        <button
          type="button"
          onClick={loadItems}
      ...
      <div className="grid gap-5 lg:grid-cols-5">
        <form className="space-y-2 rounded-2xl border border-ink/10 bg-ink/[0.02] p-4 dark:border-white/15 dark:bg-white/5 lg:col-span-2" onSubmit={handleSubmit}>
          ...
          <input
            ...
            placeholder="Facebook / Social (ঐচ্ছিক)"
            className="w-full rounded-lg border border-ink/20 bg-white px-3 py-2 text-sm outline-none dark:border-white/25 dark:bg-white/10 dark:text-white"
          />
          <textarea
            rows={3}
            ...
            placeholder="সংক্ষিপ্ত পরিচিতি (ঐচ্ছিক)"
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-ink/15 text-left text-ink/70 dark:border-white/20 dark:text-white/75">
                ...
                <th className="px-2 py-2">মিডিয়া</th>
              ...
              {isLoading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-b border-ink/10 dark:border-white/10">
                    ...
                    <td className="px-2 py-2">{item.media || '—'}</td>
